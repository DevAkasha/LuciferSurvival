            ...
            EditorGUI.indentLevel++;
            {
                ...
                this.sceneViewScrollPosition = EditorGUILayout.BeginScrollView(this.sceneViewScrollPosition, "box", GUILayout.Width(200));
                {
                    ...
                    foreach (var data in LocaleDataSO.Instance.localeData)
                    {
                        var name = data.Key;
                        /*
                        bool opened = index / 100 == selectedOpenIndex;
                        if (index % 100 == 0)
                        {
                            opened = EditorGUILayout.Foldout(opened, $"{index} ~ {index + 99}");
                            if(opened)
                            {
                                selectedOpenIndex = index / 100;
                            }
                        }
                        if (opened)
                        {
                            if (data != null)
                            {
                                ...
                                style = new GUIStyle(UnityEngine.GUI.skin.button);
                                style.alignment = TextAnchor.MiddleRight;
                                var selection = GUILayout.Toggle(selectIndex == index, new GUIContent(
                                    data.Korean.Substring(0, Mathf.Min(data.Korean.Length, 7)) + "(" + name.Substring(0, Mathf.Min(name.Length, 10)) + ")")
                                    , style);
                        ...
                        */
                        list.Add(data.Korean.Substring(0, Mathf.Min(data.Korean.Length, 7)) + "(" + name.Substring(0, Mathf.Min(name.Length, 10)) + ")");
                        index++;
                    }
                    var style = new GUIStyle(UnityEngine.GUI.skin.button);
                    style.alignment = TextAnchor.MiddleRight;
                    selectIndex = GUILayout.SelectionGrid(selectIndex, list.ToArray(), 1, style);
                    EditorGUI.indentLevel += 2;
                }
