...
namespace Ironcow.WebSocketPacket
{
    ...
    public abstract class TCPSocketManagerBase<T> : MonoSingleton<T> where T : TCPSocketManagerBase<T>
    {
        ...
        public Dictionary<int, Action<GamePacket>> _onRecv = new Dictionary<int, Action<GamePacket>>();
        ...
        bool isInit = false;
        /// <summary>
        /// 리플렉션으로 해당 클래스에 있는 메소드를 ePacketType에 맞춰 이벤트 등록
        /// </summary>
        protected void InitPackets()
        {
            if (isInit) return;
            var payloads = Enum.GetNames(typeof(ePacketType));
            var methods = GetType().GetMethods();
            foreach (var payload in payloads)
            {
                var val = (ePacketType)Enum.Parse(typeof(ePacketType), payload);
                ...
                while (socket.Connected && isConnected)
                {
                    try
                    {
                        ...
                        while (processedLength < newBuffer.Length)
                        {
                            ...
                            var type = (ePacketType)BitConverter.ToInt16(typeBytes);
