...
namespace Ironcow
{
    public static partial class Util
    {
        ...
        public static Vector3 Spot_RendomPosition(Vector3 point, float range)
        {
            Vector3 d = UnityEngine.Random.insideUnitSphere;

            point = point + (new Vector3(d.x, 0f, d.z) * range);
            return point;
        }

        public static float NormalToDegree(Vector3 normal)
        {
            return Mathf.Atan2(normal.x, normal.y) * Mathf.Rad2Deg;
        }

        public static Vector3 DegreeToNormal(float angle)
        {
